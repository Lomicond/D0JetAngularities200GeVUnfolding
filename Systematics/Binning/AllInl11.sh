...
# -------------------------
# Fixed angular binning for all observables except l11 (index 0)
# -------------------------
...
# =========================
if [[ ! -f "${SUMMARY}" ]]; then
  printf "run_id\tl11_reco_max\tl11_dense_bins\tl11_reco_n_bins\tl11_true_max\tl11_true_n_bins\tl11_reco_edges\tl11_true_edges\n" > "${SUMMARY}"
fi
...
for L11_RECO_MAX in "${L11_RECO_MAX_LIST[@]}"; do
  for L11_DENSE_BINS in "${L11_DENSE_BIN_COUNTS[@]}"; do
    ...
    while IFS= read -r L11_RECO_EDGES; do
      [[ -n "${L11_RECO_EDGES}" ]] || continue

      # Počet reco binů = počet hran - 1
      L11_RECO_N_BINS=$(awk -F',' '{print NF-1}' <<< "${L11_RECO_EDGES}")

      for L11_TRUE_MAX in "${L11_TRUE_MAX_LIST[@]}"; do
        L11_TRUE_EDGES=$(python3 - "${L11_TRUE_MAX}" <<'PY'
import sys
...
PY
        ...
        if (( L11_RECO_N_BINS < L11_TRUE_N_BINS )); then
          echo "[skip] l11 reco max=${L11_RECO_MAX}, dense=${L11_DENSE_BINS} -> Nreco=${L11_RECO_N_BINS} < Ntrue=${L11_TRUE_N_BINS}"
          continue
        fi
        ...
        cat > "${OVR_FILE}" <<EOF2
{
  ...
  // ---------- l11 scan (index 0), same for all centralities ----------
  angRecoBinsVec[0][${L11_INDEX}] = std::vector<double>{ ${L11_RECO_EDGES} };
  angRecoBinsVec[1][${L11_INDEX}] = std::vector<double>{ ${L11_RECO_EDGES} };
  angRecoBinsVec[2][${L11_INDEX}] = std::vector<double>{ ${L11_RECO_EDGES} };

  angMcBinsVecCustom[0][${L11_INDEX}] = std::vector<double>{ ${L11_TRUE_EDGES} };
  angMcBinsVecCustom[1][${L11_INDEX}] = std::vector<double>{ ${L11_TRUE_EDGES} };
  angMcBinsVecCustom[2][${L11_INDEX}] = std::vector<double>{ ${L11_TRUE_EDGES} };
...
EOF2
        ...
        echo "[${RUN_ID}] l11 reco max=${L11_RECO_MAX}, dense=${L11_DENSE_BINS}, Nreco=${L11_RECO_N_BINS} | l11 true max=${L11_TRUE_MAX}"
      ...
      python3 - "${L11_RECO_MAX}" "${L11_DENSE_BINS}" "${L11_MIN_WIDTH}" "${L11_TRUE_N_BINS}" <<'PY'
import sys
...
def rec(prev, used, widths):
    # tail bin count = len(widths)+1 because final residual bin is still missing
    ...
    # or add another regular coarse bin from allowed set
    for w in allowed:
