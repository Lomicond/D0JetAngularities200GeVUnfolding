...
BINNING_MODE_COLUMNS = {
    "Reco edges": ["reco_edges"],
    "True edges": ["true_edges"],
    "Reco + true edges": ["reco_edges", "true_edges"],
}
...
def parse_edge_list(value) -> np.ndarray:
    """Parse a summary edge field such as '[1, 3, 5]' or '1 3 5'."""
    if value is None:
    ...
    return np.asarray([float(x) for x in EDGE_NUMBER_RE.findall(text)], dtype=float)


def is_contiguous_edge_slice(candidate, reference, rtol=1e-9, atol=1e-9) -> bool:
    """Return True when candidate is reference with only end edges removed."""
    candidate = np.asarray(candidate, dtype=float).reshape(-1)
    reference = np.asarray(reference, dtype=float).reshape(-1)

    if len(candidate) < 2 or len(candidate) > len(reference):
        return False
    ...
    for start in range(len(reference) - n + 1):
        if np.allclose(candidate, reference[start:start + n], rtol=rtol, atol=atol):
            return True
    return False


def format_edge_list(edges) -> str:
    return ", ".join(f"{float(x):.8g}" for x in np.asarray(edges, dtype=float))
...
class ReferenceLoadCancelled(Exception):
    """Raised when the user declines loading a reference outside the filters."""


class MultiSelectDropdown(ttk.Menubutton):
    """Compact dropdown with checkbuttons; no selection means no restriction."""

    def __init__(self, master, width=18, all_label="(all)", **kwargs):
        self.all_label = all_label
        self.display_var = tk.StringVar(value=all_label)
        super().__init__(master, textvariable=self.display_var, width=width, **kwargs)
    ...
    def set_values(self, values):
        ...
        for value in self.values:
            self.menu.add_checkbutton(
                label=value,
                variable=self.value_vars[value],
    ...
    def _toggle_all(self):
        if self.all_var.get():
            for var in self.value_vars.values():
                var.set(False)
        elif not self.get_selected_values():
            self.all_var.set(True)
        self._update_display()

    def _toggle_value(self, _value):
        selected = self.get_selected_values()
        self.all_var.set(not selected)
        self._update_display()
    ...
    def _update_display(self):
        ...
        if not selected:
            self.all_var.set(True)
            self.display_var.set(self.all_label)
            return

        self.all_var.set(False)
        label = ", ".join(selected)
        self.display_var.set(label if len(label) <= 32 else f"{len(selected)} selected")

    def get_selected_values(self):
        return [
            value for value in self.values
            if self.value_vars[value].get()
        ]
...
class StabilityBrowser(tk.Tk):
    def __init__(self, summary_path="", stability_path=""):
        super().__init__()
        self.title("Stability browser v13")
        ...
        self.binning_ref_run_var = tk.StringVar(value="(none)")
        self.binning_edge_mode_var = tk.StringVar(value=BINNING_EDGE_MODES[0])
        self.binning_preview_var = tk.StringVar(value="Inactive.")
        ...
        self._external_reference_approvals = set()
    ...
    def _build_ui(self):
        ...
        filters = ttk.LabelFrame(self, text="Filters (select one or more values)")
        ...
        binning_frame = ttk.LabelFrame(filters, text="Binning style")
        binning_frame.grid(row=1, column=0, columnspan=6, sticky="ew", padx=4, pady=6)
        binning_frame.columnconfigure(1, weight=1)
        binning_frame.columnconfigure(3, weight=1)

        ttk.Label(binning_frame, text="Reference run").grid(row=0, column=0, sticky="w", padx=4, pady=3)
        self.binning_ref_run_cb = ttk.Combobox(
            binning_frame,
            textvariable=self.binning_ref_run_var,
            values=["(none)"],
            state="normal",
            width=18,
        )
        self.binning_ref_run_cb.grid(row=0, column=1, sticky="ew", padx=4, pady=3)
        self.binning_ref_run_cb.bind("<<ComboboxSelected>>", self.update_binning_preview)

        ttk.Label(binning_frame, text="Compare").grid(row=0, column=2, sticky="w", padx=4, pady=3)
        self.binning_edge_mode_cb = ttk.Combobox(
            binning_frame,
            textvariable=self.binning_edge_mode_var,
            values=BINNING_EDGE_MODES,
            state="readonly",
            width=22,
        )
        self.binning_edge_mode_cb.grid(row=0, column=3, sticky="ew", padx=4, pady=3)
        self.binning_edge_mode_cb.bind("<<ComboboxSelected>>", self.update_binning_preview)

        ttk.Button(
            binning_frame,
            text="Use selected row",
            command=self.use_selected_row_as_binning_reference,
        ).grid(row=0, column=4, sticky="ew", padx=4, pady=3)

        ttk.Label(
            binning_frame,
            textvariable=self.binning_preview_var,
            wraplength=1450,
        ).grid(row=1, column=0, columnspan=5, sticky="w", padx=4, pady=3)

        metric_frame = ttk.LabelFrame(filters, text="Metric ranges by row")
        metric_frame.grid(row=2, column=0, columnspan=6, sticky="ew", padx=4, pady=6)
        ...
        cross_frame.grid(row=3, column=0, columnspan=6, sticky="ew", padx=4, pady=6)
        for i in range(2):
        ...
        btns.grid(row=4, column=0, columnspan=6, sticky="ew", padx=4, pady=4)
    ...
    def rebuild_filter_widgets(self):
        ...
        for idx, col in enumerate(self.filter_cols):
            ...
            dropdown = MultiSelectDropdown(box_frame, width=18)
            dropdown.pack(fill="x")
            self.filter_boxes[col] = dropdown
    ...
    def populate_filter_boxes(self):
        ...
        for col, dropdown in self.filter_boxes.items():
            vals = self.df_all[col].dropna().unique().tolist()
            vals = [str(v) for v in vals if str(v) != ""]
            vals = sorted(vals, key=self._sort_value_key)
            dropdown.set_values(vals)

        run_ids = sorted(
            self.df_all["run_id"].dropna().astype(str).unique().tolist(),
            key=run_id_to_int,
        )
        self.binning_ref_run_cb["values"] = ["(none)"] + run_ids
        self.binning_ref_run_var.set("(none)")
        self.binning_edge_mode_var.set(BINNING_EDGE_MODES[0])
        self.update_binning_preview()
    ...
    def resolve_binning_reference_run(self, run_text: str) -> str:
        text = str(run_text).strip()
        if not text or text == "(none)":
            return ""

        run_ids = self.df_all["run_id"].dropna().astype(str).unique().tolist()
        if text in run_ids:
            return text
        ...
        run_number = run_id_to_int(text)
        number_matches = (
            [r for r in run_ids if run_id_to_int(r) == run_number]
            if run_number >= 0 else []
        )
        if run_number >= 0 and len(number_matches) == 1:
            return number_matches[0]
        if len(number_matches) > 1:
            raise ValueError(
                f"Binning reference '{text}' is ambiguous: "
                + ", ".join(sorted(number_matches, key=run_id_to_int))
            )

        raise ValueError(f"Binning reference run '{text}' was not found in summary.tsv.")

    def get_binning_reference(self):
        ref_run = self.resolve_binning_reference_run(self.binning_ref_run_var.get())
        if not ref_run:
            return "", [], None

        mode = self.binning_edge_mode_var.get().strip()
        if mode not in BINNING_MODE_COLUMNS:
            raise ValueError(f"Unknown binning comparison mode: {mode}")

        ref_rows = self.df_all[self.df_all["run_id"].astype(str) == ref_run]
        if ref_rows.empty:
            raise ValueError(f"Binning reference run {ref_run} was not found.")
        ref_row = ref_rows.iloc[0]

        edge_columns = BINNING_MODE_COLUMNS[mode]
        for col in edge_columns:
            if col not in self.df_all.columns:
                raise ValueError(
                    f"The selected mode needs column '{col}', but it is not present in summary.tsv."
                )
            edges = parse_edge_list(ref_row[col])
            if len(edges) < 2:
                raise ValueError(
                    f"Reference run {ref_run} has no valid {col} definition."
                )

        return ref_run, edge_columns, ref_row

    def update_binning_preview(self, _event=None):
        if self.df_all.empty:
            self.binning_preview_var.set("Inactive.")
            return

        try:
            ref_run, edge_columns, ref_row = self.get_binning_reference()
            if not ref_run:
                self.binning_preview_var.set(
                    "Inactive. Select a reference run to keep only contiguous slices of its edges."
                )
                return

            parts = []
            for col in edge_columns:
                parts.append(f"{col}: {format_edge_list(parse_edge_list(ref_row[col]))}")
            self.binning_preview_var.set(
                f"Reference {ref_run} | " + " | ".join(parts)
                + " | candidates may remove edges only from the left and/or right end."
            )
        except ValueError as e:
            self.binning_preview_var.set(f"Invalid binning reference: {e}")

    def use_selected_row_as_binning_reference(self):
        selection = self.result_tree.selection()
        if not selection:
            messagebox.showinfo("Info", "Select a row in the matching-results table first.")
            return
        ...
        self.binning_ref_run_var.set(str(self.df_filtered.iloc[idx]["run_id"]))
        self.update_binning_preview()

    def apply_binning_style_filter(self, df: pd.DataFrame):
        ref_run, edge_columns, ref_row = self.get_binning_reference()
        if not ref_run:
            return df, ""
        ...
        out = df
        for col in edge_columns:
            reference_edges = parse_edge_list(ref_row[col])
            match_cache = {}

            def matches(value):
                key = str(value)
                if key not in match_cache:
                    candidate_edges = parse_edge_list(value)
                    match_cache[key] = is_contiguous_edge_slice(
                        candidate_edges, reference_edges
                    )
                return match_cache[key]

            mask = out[col].map(matches).astype(bool)
            out = out[mask]

        mode = self.binning_edge_mode_var.get().strip()
        note = (
            f" Binning style {mode} from {ref_run} kept "
            f"{out['run_id'].nunique()} runs."
        )
        self.binning_ref_run_var.set(ref_run)
        self.update_binning_preview()
        return out, note
    ...
    def apply_filters(self):
        ...
        try:
            binning_note = ""
            for col, dropdown in self.filter_boxes.items():
                selected = dropdown.get_selected_values()
                if selected:
                    df = df[df[col].astype(str).isin(selected)]

            df, binning_note = self.apply_binning_style_filter(df)
    ...
    def reset_filters(self):
        for dropdown in self.filter_boxes.values():
            dropdown.reset()
        self.binning_ref_run_var.set("(none)")
        self.binning_edge_mode_var.set(BINNING_EDGE_MODES[0])
        self.update_binning_preview()
    ...
    def format_root_path(self, pattern: str, run_id: str) -> str:
        try:
            return pattern.format(run_id=run_id)
        except Exception as e:
            raise ValueError(
                f"Could not apply ROOT file pattern to run {run_id}:\n{e}"
            )

    def confirm_reference_outside_filter(self, ref_run: str, run_ids, pattern: str) -> bool:
        """Confirm once before loading a fixed reference excluded by the filters."""
        if ref_run in run_ids:
            return False

        root_path = self.format_root_path(pattern, ref_run)
        approval_key = (ref_run, pattern)
        if approval_key not in self._external_reference_approvals:
            proceed = messagebox.askyesno(
                "Reference run outside current filters",
                f"Reference run {ref_run} is not among the runs selected by the "
                "current filters. It may have been filtered out.\n\n"
                "Try to load it directly using the ROOT file pattern?\n\n"
                f"{root_path}\n\n"
                "It will be used as the fixed reference, but it will not be added "
                "to the filtered table.",
                parent=self,
            )
            if not proceed:
                self.results_status_var.set(
                    f"Loading reference {ref_run} outside the filters was cancelled."
                )
                raise ReferenceLoadCancelled()
            self._external_reference_approvals.add(approval_key)
    ...
    def collect_histograms_for_filtered_runs(self):
        ...
        reference_outside_filter = self.confirm_reference_outside_filter(
            ref_run, run_ids, pattern
        )
        ...
        for idx, run_id in enumerate(run_ids, start=1):
            root_path = self.format_root_path(pattern, run_id)
            try:
        ...
        if reference_outside_filter:
            root_path = self.format_root_path(pattern, ref_run)
            self.results_status_var.set(
                f"Loading fixed reference {ref_run} outside the current filters..."
            )
            self.update_idletasks()
            try:
                edges, values = read_hist_arrays(root_path, hist_name)
                ...
                raise ValueError(
                    f"Reference run {ref_run} was outside the current filters and "
                    "could not be loaded separately.\n"
                    f"ROOT file: {root_path}\n"
                    f"{e}"
                )
        ...
        reference_note = " (outside filter)" if reference_outside_filter else ""
        self.results_status_var.set(
            f"Histogram {hist_name}: loaded {len(data)} runs, skipped {len(skipped)}. "
            f"Reference = {ref_run}{reference_note}."
        )
        ...
        return {
            ...
            "meta": meta,
            "skipped": skipped,
            "reference_outside_filter": reference_outside_filter,
        }
    ...
    def collect_iteration_variations_for_reference_run(self):
        ...
        self.results_status_var.set(
            ...
            + (", reference outside filter." if reference_outside_filter else ".")
        )
