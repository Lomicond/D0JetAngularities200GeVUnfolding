...
def load_stability(path: str) -> pd.DataFrame:
    ...
    if ncols < len(base_cols):
        raise ValueError(
            f"stability.tsv ma jen {ncols} sloupcu, ale ocekavam aspon {len(base_cols)}."
        )
    if ncols > len(all_cols):
        raise ValueError(
            f"stability.tsv ma {ncols} sloupcu, ale skript umi nejvyse {len(all_cols)}."
        )
...
def load_summary(path: str) -> pd.DataFrame:
    ...
    if "run_id" not in cols:
        raise ValueError(
            "summary.tsv musi obsahovat sloupec 'run_id'.\n"
            f"Nalezene sloupce:\n{cols}"
        )

    # puvodni podporovane formaty prevedeme na jednotne nazvy
    if cols == SUMMARY_CANONICAL_COLS:
        ...
        # obecny format: ponech vsechny sloupce tak, jak jsou v summary
        df = raw.copy()

    df["run_id"] = df["run_id"].astype(str)

    # 'edges' sloupce nech jako text, ostatni preved na cisla jen tehdy,
    # kdyz jsou opravdu cele numericke
    for c in df.columns:
        if c == "run_id":
            continue
        if "edges" in c.lower():
...
def build_result_hist_name(kind_label: str, cent: int, iteration_display: int, method: str) -> str:
    ...
    if kind_label not in RESULT_HIST_KIND_MAP:
        raise KeyError(f"Neznamy ROOT result: {kind_label}")
    if iteration_display < 1:
        raise ValueError("Iterace se zadava 1-based: napr. 4 nacte ROOT histogram *_it3_*." )
...
def read_hist_arrays(root_path: str, hist_name: str):
    if uproot is not None:
        with uproot.open(root_path) as f:
            ...
            if key is None:
                raise KeyError(f"Histogram {hist_name} nenalezen v {root_path}")
            h = f[key]
    ...
    if ROOT is not None:
        ...
        if not f or f.IsZombie():
            raise OSError(f"Nelze otevrit ROOT soubor: {root_path}")
        try:
            obj = f.Get(hist_name)
            if obj is None:
                key = None
    ...
    raise ImportError("Neni dostupny ani uproot, ani ROOT (PyROOT).")


class StabilityBrowser(tk.Tk):
    def __init__(self, summary_path="", stability_path=""):
        super().__init__()
        self.title("Stability browser")
    ...
    def _build_ui(self):
        ...
        top = ttk.LabelFrame(self, text="Vstupni soubory")
        ...
        ttk.Button(top, text="Vybrat", command=self.browse_summary).grid(row=0, column=2, padx=4, pady=4)
        ...
        ttk.Button(top, text="Vybrat", command=self.browse_stability).grid(row=0, column=5, padx=4, pady=4)

        ttk.Button(top, text="Nacist", command=self.load_files).grid(row=0, column=6, padx=10, pady=4)
        ...
        metric_frame = ttk.LabelFrame(filters, text="Rozsahy metrik po radcich")
        ...
        cross_frame = ttk.LabelFrame(filters, text="Krizove filtry na stejnem (run_id, cent)")
        ...
        for idx, tag in enumerate(("A", "B")):
            ...
            ttk.Label(lf, text="Metrika").grid(row=2, column=0, sticky="w", padx=4, pady=2)
        ...
        ttk.Label(mode_frame, text="Po krizovem filtru zobrazit").pack(side="left", padx=4)
        ttk.Combobox(
            mode_frame,
        ...
        ttk.Button(btns, text="Pouzit filtry", command=self.apply_filters).pack(side="left", padx=4)
        ttk.Button(btns, text="Reset filtru", command=self.reset_filters).pack(side="left", padx=4)
        ttk.Button(btns, text="Export filtrovaneho CSV", command=self.export_filtered_csv).pack(side="left", padx=4)

        self.status_var = tk.StringVar(value="Nenahrana data.")
        ttk.Label(btns, textvariable=self.status_var).pack(side="right", padx=4)

        result_frame = ttk.LabelFrame(self, text="Vyhovujici radky")
        ...
        bottom.add(detail_tab, text="Detail vybraneho radku")

        agg_tab = ttk.Frame(bottom)
        agg_tab.columnconfigure(1, weight=1)
        agg_tab.rowconfigure(1, weight=1)
        bottom.add(agg_tab, text="Agregace / mapovani vlivu")

        left_agg = ttk.LabelFrame(agg_tab, text="Nastaveni agregace")
        left_agg.grid(row=0, column=0, sticky="nsw", padx=4, pady=4)

        ttk.Label(left_agg, text="Group by (Ctrl/Shift pro vice)").grid(row=0, column=0, sticky="w", padx=4, pady=4)
        ...
        ttk.Label(left_agg, text="Metrika").grid(row=2, column=0, sticky="w", padx=4, pady=2)
        ...
        ttk.Label(left_agg, text="Agregace").grid(row=4, column=0, sticky="w", padx=4, pady=2)
        ...
        ttk.Button(left_agg, text="Spocitat agregaci", command=self.compute_group_summary).grid(row=6, column=0, sticky="ew", padx=4, pady=8)
        ttk.Button(left_agg, text="Export agregace CSV", command=self.export_group_csv).grid(row=7, column=0, sticky="ew", padx=4, pady=2)

        right_agg = ttk.LabelFrame(agg_tab, text="Vysledek agregace")
        ...
        res_top = ttk.LabelFrame(results_tab, text="ROOT vysledky")
        ...
        ttk.Label(res_top, text="Centralita").grid(row=1, column=2, sticky="w", padx=4, pady=2)
        ...
        ttk.Label(res_top, text="Iterace (zadej 4 -> it3)").grid(row=1, column=4, sticky="w", padx=4, pady=2)
        ...
        ttk.Label(res_top, text="Referencni run").grid(row=2, column=2, sticky="w", padx=4, pady=2)
    ...
    def load_files(self):
        ...
        if not summary_path or not stability_path:
            messagebox.showerror("Chyba", "Vyber summary.tsv i stability.tsv.")
            return
        ...
        try:
            ...
            messagebox.showerror("Chyba pri nacitani", str(e))
    ...
    def _parse_metric_bounds(self, min_text: str, max_text: str, label: str):
        ...
        if min_text.strip():
            try:
                ...
                raise ValueError(f"Neplatne minimum pro {label}: {min_text}")

        if max_text.strip():
            try:
                out_max = float(max_text)
            except ValueError:
                raise ValueError(f"Neplatne maximum pro {label}: {max_text}")
    ...
    def apply_filters(self):
        if self.df_all.empty:
            self.status_var.set("Nenahrana data.")
            return
        ...
        try:
            ...
            if active_a or active_b:
                if df.empty:
                    ...
                    self.status_var.set(f"Nacteno {len(self.df_all)} radku, filtr vybral 0 radku.")
                    return
            ...
            msg = f"Nacteno {len(self.df_all)} radku, filtr vybral {len(self.df_filtered)} radku."
            if groups_kept is not None:
                msg += f" Krizovy filtr ponechal {len(groups_kept)} skupin (run_id, cent)."
    ...
    def compute_group_summary(self):
        if self.df_filtered.empty:
            messagebox.showinfo("Info", "Nejsou zadna filtrovana data.")
            return
        ...
        if metric not in METRIC_COLS:
            messagebox.showerror("Chyba", "Vyber platnou metriku.")
            return
    ...
    def export_filtered_csv(self):
        if self.df_filtered.empty:
            messagebox.showinfo("Info", "Neni co exportovat.")
            return

        path = filedialog.asksaveasfilename(
            title="Ulozit filtrovana data",
        ...
        messagebox.showinfo("Hotovo", f"Filtrovana data ulozena do:\n{path}")

    def export_group_csv(self):
        if self.group_df.empty:
            messagebox.showinfo("Info", "Nejdriv spocitej agregaci.")
            return

        path = filedialog.asksaveasfilename(
            title="Ulozit agregaci",
            defaultextension=".csv",
            filetypes=[("CSV files", "*.csv"), ("All files", "*.*")]
        )
        if not path:
        ...
        messagebox.showinfo("Hotovo", f"Agregace ulozena do:\n{path}")
    ...
    def get_results_hist_name(self):
        try:
            ...
            raise ValueError("Centralita i iterace musi byt cela cisla. Iterace se zadava 1-based: napr. 4 nacte *_it3_*.")
        kind_label = self.results_kind_var.get().strip()
        method = self.results_method_var.get().strip()
        if not kind_label or not method:
            raise ValueError("ROOT result a Method musi byt vyplnene.")
        if kind_label not in RESULT_HIST_KIND_MAP:
            raise ValueError(f"Neznamy ROOT result: {kind_label}")
        return build_result_hist_name(kind_label, cent, iteration, method)

    def collect_histograms_for_filtered_runs(self):
        run_ids = self.get_filtered_run_ids()
        if not run_ids:
            raise ValueError("Po aktualnich filtrech nejsou k dispozici zadne runy.")

        hist_name = self.get_results_hist_name()
        pattern = self.results_root_pattern_var.get().strip()
        if not pattern:
            raise ValueError("ROOT file pattern nesmi byt prazdny.")
        ...
        self.results_status_var.set(f"Nacitam {len(run_ids)} ROOT souboru pro {hist_name}...")
        ...
        for idx, run_id in enumerate(run_ids, start=1):
            ...
            if idx == 1 or idx % 10 == 0 or idx == len(run_ids):
                self.results_status_var.set(f"Nacteno {idx}/{len(run_ids)} souboru, uspesne {len(data)}, skipped {len(skipped)}")
                self.update_idletasks()

        if ref_run not in data:
            raise ValueError(
                f"Referencni run {ref_run} se nepodarilo nacist.\n"
        ...
        self.results_status_var.set(
            f"Histogram {hist_name}: nacteno {len(data)} runu, skipped {len(skipped)}. "
            f"Reference = {ref_run}."
        )
    ...
    def collect_iteration_variations_for_reference_run(self):
        ...
        if not ref_run:
            if not run_ids:
                raise ValueError("Neni vybran referencni run a po filtrech nejsou k dispozici zadne runy.")
            ref_run = run_ids[0]
            self.results_ref_run_var.set(ref_run)
        ...
        self.results_status_var.set(
            f"Nacitam iterace {iter_candidates} pro {ref_run}..."
        )
        self.update_idletasks()
        ...
        if iteration_nominal not in data:
            raise ValueError(
                f"Nominalni iterace {iteration_nominal} se nepodarila nacist.\n"
                + ("\n".join(skipped[:8]) if skipped else "")
            )
        ...
        if not variation_iters:
            raise ValueError(
                "Nepodarilo se nacist zadnou sousedni iteraci.\n"
                + ("\n".join(skipped[:8]) if skipped else "")
            )
    ...
    def plot_bin_evolution(self):
        ...
        if not run_ids:
            messagebox.showerror("Chyba", "Nenalezena zadna nacitana data pro vyvoj binu.")
            return
    ...
    def compute_reference_systematics_by_bin(self, pack):
        ...
        if not alt_runs:
            raise ValueError("Pro vypocet systematiky je potreba aspon jeden alternativni run mimo referenci.")
        ...
        for ib in range(len(ref_values)):
            ...
            if diffs.size == 0:
                ...
                # Symmetric RMS odchylka vuci zvolenemu referencnimu binningu,
                # ne std vuci mean.
                sigma_pct = float(np.sqrt(np.mean(diffs * diffs)))

                # Asymetricka RMS varianta: zvlast zaporne a kladne odchylky.
                # Hodnoty ukladame jako kladne velikosti systematicke chyby.
                ...
                # Envelope take ukladame jako kladne velikosti, ne jako podepsane hodnoty.
                env_min = float(np.min(diffs))
                env_max = float(np.max(diffs))
                envelope_down_pct = max(0.0, -env_min)
                envelope_up_pct = max(0.0, env_max)
    ...
    def copy_dataframe_to_clipboard(self, df: pd.DataFrame):
        if df is None or df.empty:
            messagebox.showinfo("Info", "Neni co kopirovat.")
            return
        txt = df.to_csv(sep="\t", index=False, float_format="%.8g")
        self.clipboard_clear()
        self.clipboard_append(txt)
        self.update()
        messagebox.showinfo("Hotovo", "Tabulka byla zkopirovana do schranky jako TSV.")
    ...
    def export_dataframe_csv(self, df: pd.DataFrame, default_name="table.csv"):
        ...
        messagebox.showinfo("Hotovo", f"Tabulka ulozena do:\n{path}")
    ...
    def plot_heatmap(self):
        ...
        try:
            ...
            messagebox.showerror("Chyba", "Heatmap bin musi byt cele cislo (1-based).")
            return

        ref_edges = pack["ref_edges"]
        n_bins = len(ref_edges) - 1
        if bin_index < 1 or bin_index > n_bins:
            messagebox.showerror("Chyba", f"Heatmap bin musi byt mezi 1 a {n_bins}.")
            return
        ...
        if x_col not in self.heatmap_param_choices or y_col not in self.heatmap_param_choices:
            messagebox.showerror("Chyba", "Vyber platne parametry heatmapy.")
            return
        if x_col == y_col:
            messagebox.showerror("Chyba", "Heatmap X a Y musi byt ruzne parametry.")
            return
        ...
        if hdf.empty:
            messagebox.showerror("Chyba", "Pro heatmapu nejsou k dispozici zadna data.")
            return

        pivot = hdf.pivot_table(index=y_col, columns=x_col, values="rel_diff_pct", aggfunc="mean")
        if pivot.empty:
            messagebox.showerror("Chyba", "Heatmap pivot je prazdny.")
            return
...
def parse_args():
    p = argparse.ArgumentParser(
        description=(
            "Interaktivni prohlizec pro summary.tsv a stability.tsv. "
            "Umoznuje filtrovat podle reco/true rozsahu, podle limitu na metriky "
            "a navic pres dvojici krizovych filtru A/B na stejnem (run_id, cent)."
        )
    )
