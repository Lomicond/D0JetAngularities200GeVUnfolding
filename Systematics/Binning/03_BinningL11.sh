...
# -------------------------
# lambda_1^1 / l11 scan only
# -------------------------
# Index 0 corresponds to lambda_1^1 / l11.
# The override macro changes only:
#   angRecoBinsVec[*][0]
#   angMcBinsVecCustom[*][0]
# All other binnings stay exactly as defined in config.h/config_hist.h.
L11_INDEX=0

L11_TRUE_MAX_LIST=(0.4 0.6 0.8 1.0)
L11_RECO_MAX_LIST=(0.4 0.6 0.8 1.0)
L11_DENSE_BIN_COUNTS=(4 5 6)
L11_MIN_WIDTH=0.05
L11_TAIL_WIDTHS=(0.1 0.2 0.3 0.4)
L11_TRUE_N_BINS=7

# Edge nudging.
# The smallest response-matrix step for lambda11 is 0.0025,
# so the chosen shift 0.005 corresponds to two matrix steps.
# The shift is applied coherently to internal reco and true edges.
# The first and last edges remain fixed.
L11_EDGE_SHIFTS=(0 -0.005 0.005)
...
edges = [0, 0.05, 0.1, 0.15, 0.2, 0.25, 0.3, xmax]
...
# =========================
if [[ ! -f "${SUMMARY}" ]]; then
  printf "run_id\tl11_edge_shift\tl11_reco_max\tl11_dense_bins\tl11_reco_n_bins\tl11_true_max\tl11_true_n_bins\tl11_reco_edges\tl11_true_edges\toverride_file\troot_log\n" > "${SUMMARY}"
fi
...
# Main loop
# =========================
for L11_RECO_MAX in "${L11_RECO_MAX_LIST[@]}"; do
  for L11_DENSE_BINS in "${L11_DENSE_BIN_COUNTS[@]}"; do

    while IFS= read -r L11_RECO_EDGES_BASE; do
      [[ -n "${L11_RECO_EDGES_BASE}" ]] || continue

      for L11_EDGE_SHIFT in "${L11_EDGE_SHIFTS[@]}"; do
        L11_RECO_EDGES=$(shift_edges "${L11_RECO_EDGES_BASE}" "${L11_EDGE_SHIFT}")

        # Number of reco bins = number of edges - 1.
        L11_RECO_N_BINS=$(awk -F',' '{print NF-1}' <<< "${L11_RECO_EDGES}")

        for L11_TRUE_MAX in "${L11_TRUE_MAX_LIST[@]}"; do
          L11_TRUE_EDGES_BASE=$(format_edges_from_max "${L11_TRUE_MAX}")
          L11_TRUE_EDGES=$(shift_edges "${L11_TRUE_EDGES_BASE}" "${L11_EDGE_SHIFT}")
          L11_TRUE_N_BINS=7

          # The true level must not have more bins than the reco level.
          if (( L11_RECO_N_BINS < L11_TRUE_N_BINS )); then
            echo "[skip] l11 shift=${L11_EDGE_SHIFT}, reco max=${L11_RECO_MAX}, dense=${L11_DENSE_BINS} -> Nreco=${L11_RECO_N_BINS} < Ntrue=${L11_TRUE_N_BINS}"
            continue
          fi
          ...
          # Changes only lambda_1^1 / l11 binning for all centralities.
          # Uses ROOT5-friendly array + push_back syntax.
          # -------------------------
          cat > "${OVR_FILE}" <<EOF_OVR
{
  cout << "[override] l11 reco edges: ${L11_RECO_EDGES}" << endl;
  cout << "[override] l11 true edges: ${L11_TRUE_EDGES}" << endl;

  double l11_reco_edges[] = { ${L11_RECO_EDGES} };
  int nReco = (int)(sizeof(l11_reco_edges)/sizeof(double));

  double l11_true_edges[] = { ${L11_TRUE_EDGES} };
  int nTrue = (int)(sizeof(l11_true_edges)/sizeof(double));

  for (int ic = 0; ic < nCentralityBins; ++ic) {
    angRecoBinsVec[ic][${L11_INDEX}].clear();
    for (int i = 0; i < nReco; ++i) {
      angRecoBinsVec[ic][${L11_INDEX}].push_back(l11_reco_edges[i]);
    }

    angMcBinsVecCustom[ic][${L11_INDEX}].clear();
    for (int i = 0; i < nTrue; ++i) {
      angMcBinsVecCustom[ic][${L11_INDEX}].push_back(l11_true_edges[i]);
    }
  }
}
EOF_OVR

          echo "[${RUN_ID}] l11 shift=${L11_EDGE_SHIFT}, reco max=${L11_RECO_MAX}, dense=${L11_DENSE_BINS}, Nreco=${L11_RECO_N_BINS} | true max=${L11_TRUE_MAX}, Ntrue=${L11_TRUE_N_BINS}"
          ...
          printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
            "${RUN_ID}" \
            "${L11_EDGE_SHIFT}" \
            "${L11_RECO_MAX}" "${L11_DENSE_BINS}" "${L11_RECO_N_BINS}" \
            "${L11_TRUE_MAX}" "${L11_TRUE_N_BINS}" \
            "${L11_RECO_EDGES}" "${L11_TRUE_EDGES}" \
            "${OVR_FILE}" "${ROOT_LOG}" \
            >> "${SUMMARY}"
      ...
      python3 - "${L11_RECO_MAX}" "${L11_DENSE_BINS}" "${L11_MIN_WIDTH}" "${L11_TRUE_N_BINS}" "${L11_TAIL_WIDTHS[@]}" <<'PY'
import sys
