...
# -------------------------
# Fixed angular binning for all observables except l13 (index 3)
# -------------------------
...
# =========================
if [[ ! -f "${SUMMARY}" ]]; then
  printf "run_id\tl13_reco_max\tl13_dense_bins\tl13_reco_n_bins\tl13_true_max\tl13_true_n_bins\tl13_reco_edges\tl13_true_edges\n" > "${SUMMARY}"
fi
...
for L13_RECO_MAX in "${L13_RECO_MAX_LIST[@]}"; do
  for L13_DENSE_BINS in "${L13_DENSE_BIN_COUNTS[@]}"; do
    ...
    while IFS= read -r L13_RECO_EDGES; do
      [[ -n "${L13_RECO_EDGES}" ]] || continue

      L13_RECO_N_BINS=$(awk -F',' '{print NF-1}' <<< "${L13_RECO_EDGES}")

      for L13_TRUE_MAX in "${L13_TRUE_MAX_LIST[@]}"; do
        L13_TRUE_EDGES=$(python3 - "${L13_TRUE_MAX}" <<'PY'
import sys
...
PY
        ...
        if (( L13_RECO_N_BINS < L13_TRUE_N_BINS )); then
          echo "[skip] l13 reco max=${L13_RECO_MAX}, dense=${L13_DENSE_BINS} -> Nreco=${L13_RECO_N_BINS} < Ntrue=${L13_TRUE_N_BINS}"
          continue
        fi
        ...
        cat > "${OVR_FILE}" <<EOF2
{
  ...
  // ---------- l13 scan (index 3), same for all centralities ----------
  angRecoBinsVec[0][${L13_INDEX}] = std::vector<double>{ ${L13_RECO_EDGES} };
  angRecoBinsVec[1][${L13_INDEX}] = std::vector<double>{ ${L13_RECO_EDGES} };
  angRecoBinsVec[2][${L13_INDEX}] = std::vector<double>{ ${L13_RECO_EDGES} };

  angMcBinsVecCustom[0][${L13_INDEX}] = std::vector<double>{ ${L13_TRUE_EDGES} };
  angMcBinsVecCustom[1][${L13_INDEX}] = std::vector<double>{ ${L13_TRUE_EDGES} };
  angMcBinsVecCustom[2][${L13_INDEX}] = std::vector<double>{ ${L13_TRUE_EDGES} };
...
EOF2
        ...
        echo "[${RUN_ID}] l13 reco max=${L13_RECO_MAX}, dense=${L13_DENSE_BINS}, Nreco=${L13_RECO_N_BINS} | l13 true max=${L13_TRUE_MAX}"
      ...
      python3 - "${L13_RECO_MAX}" "${L13_DENSE_BINS}" "${L13_MIN_WIDTH}" "${L13_TRUE_N_BINS}" <<'PY'
import sys
