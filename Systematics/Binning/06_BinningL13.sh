...
# -------------------------
# lambda_3^1 / l13 scan only
# -------------------------
# Index 3 corresponds to lambda_3^1 / l13.
# The override macro changes only:
#   angRecoBinsVec[*][3]
#   angMcBinsVecCustom[*][3]
# All other binnings stay exactly as defined in config.h/config_hist.h.
L13_INDEX=3

L13_TRUE_MAX_LIST=(0.4 0.6 0.8 1.0)
L13_RECO_MAX_LIST=(0.4 0.6 0.8 1.0)
L13_DENSE_BIN_COUNTS=(4 5 6)
L13_MIN_WIDTH=0.0125
L13_TAIL_WIDTHS=(0.1 0.2 0.3 0.4)
L13_TRUE_N_BINS=8
...
edges = [0, 0.0125, 0.025, 0.0375, 0.05, 0.075, 0.1, 0.15, xmax]
...
# =========================
if [[ ! -f "${SUMMARY}" ]]; then
  printf "run_id\tl13_edge_shift\tl13_reco_max\tl13_dense_bins\tl13_reco_n_bins\tl13_true_max\tl13_true_n_bins\tl13_reco_edges\tl13_true_edges\toverride_file\troot_log\n" > "${SUMMARY}"
fi
...
# Main loop
# =========================
for L13_RECO_MAX in "${L13_RECO_MAX_LIST[@]}"; do
  for L13_DENSE_BINS in "${L13_DENSE_BIN_COUNTS[@]}"; do

    while IFS= read -r L13_RECO_EDGES_BASE; do
      [[ -n "${L13_RECO_EDGES_BASE}" ]] || continue

      for L13_EDGE_SHIFT in "${L13_EDGE_SHIFTS[@]}"; do
        L13_RECO_EDGES=$(shift_edges "${L13_RECO_EDGES_BASE}" "${L13_EDGE_SHIFT}")

        # Number of reco bins = number of edges - 1.
        L13_RECO_N_BINS=$(awk -F',' '{print NF-1}' <<< "${L13_RECO_EDGES}")
        ...
        for L13_TRUE_MAX in "${L13_TRUE_MAX_LIST[@]}"; do
          ...
          if (( L13_RECO_N_BINS < L13_TRUE_N_BINS )); then
            echo "[skip] l13 shift=${L13_EDGE_SHIFT}, reco max=${L13_RECO_MAX}, dense=${L13_DENSE_BINS} -> Nreco=${L13_RECO_N_BINS} < Ntrue=${L13_TRUE_N_BINS}"
            continue
          fi
...
{
  cout << "[override] lambda_3^1 / l13 reco edges: ${L13_RECO_EDGES}" << endl;
  cout << "[override] lambda_3^1 / l13 true edges: ${L13_TRUE_EDGES}" << endl;

  // lambda_3^1 / l13 scan only, index ${L13_INDEX}.
  // All other binning variables stay exactly as defined in config.h/config_hist.h.

  double reco_edges[] = { ${L13_RECO_EDGES} };
  int nReco = (int)(sizeof(reco_edges)/sizeof(double));
  for (int ic = 0; ic < nCentralityBins; ++ic) {
    angRecoBinsVec[ic][${L13_INDEX}].clear();
    for (int i = 0; i < nReco; ++i) {
      angRecoBinsVec[ic][${L13_INDEX}].push_back(reco_edges[i]);
    }
  }

  double true_edges[] = { ${L13_TRUE_EDGES} };
  int nTrue = (int)(sizeof(true_edges)/sizeof(double));
  for (int ic = 0; ic < nCentralityBins; ++ic) {
    angMcBinsVecCustom[ic][${L13_INDEX}].clear();
    for (int i = 0; i < nTrue; ++i) {
      angMcBinsVecCustom[ic][${L13_INDEX}].push_back(true_edges[i]);
    }
  }
}
EOF_OVR

          echo "[${RUN_ID}] l13 shift=${L13_EDGE_SHIFT}, reco max=${L13_RECO_MAX}, dense=${L13_DENSE_BINS}, Nreco=${L13_RECO_N_BINS} | true max=${L13_TRUE_MAX}"
          ...
          printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
            "${RUN_ID}" \
            "${L13_EDGE_SHIFT}" \
            "${L13_RECO_MAX}" "${L13_DENSE_BINS}" "${L13_RECO_N_BINS}" \
            "${L13_TRUE_MAX}" "${L13_TRUE_N_BINS}" \
            "${L13_RECO_EDGES}" "${L13_TRUE_EDGES}" \
            "${OVR_FILE}" "${ROOT_LOG}" \
            >> "${SUMMARY}"
      ...
      python3 - "${L13_RECO_MAX}" "${L13_DENSE_BINS}" "${L13_MIN_WIDTH}" "${L13_TRUE_N_BINS}" "${L13_TAIL_WIDTHS[@]}" <<'PY'
import sys
