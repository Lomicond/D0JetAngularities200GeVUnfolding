...
# -------------------------
# Fixed angular binning for all observables except l11.5 (index 1)
# -------------------------
...
# =========================
if [[ ! -f "${SUMMARY}" ]]; then
  printf "run_id\tl11p5_reco_max\tl11p5_dense_bins\tl11p5_reco_n_bins\tl11p5_true_max\tl11p5_true_n_bins\tl11p5_reco_edges\tl11p5_true_edges\n" > "${SUMMARY}"
fi
...
for L11P5_RECO_MAX in "${L11P5_RECO_MAX_LIST[@]}"; do
  for L11P5_DENSE_BINS in "${L11P5_DENSE_BIN_COUNTS[@]}"; do
    ...
    while IFS= read -r L11P5_RECO_EDGES; do
      [[ -n "${L11P5_RECO_EDGES}" ]] || continue

      L11P5_RECO_N_BINS=$(awk -F',' '{print NF-1}' <<< "${L11P5_RECO_EDGES}")

      for L11P5_TRUE_MAX in "${L11P5_TRUE_MAX_LIST[@]}"; do
        L11P5_TRUE_EDGES=$(python3 - "${L11P5_TRUE_MAX}" <<'PY'
import sys
...
PY
        ...
        if (( L11P5_RECO_N_BINS < L11P5_TRUE_N_BINS )); then
          echo "[skip] l11.5 reco max=${L11P5_RECO_MAX}, dense=${L11P5_DENSE_BINS} -> Nreco=${L11P5_RECO_N_BINS} < Ntrue=${L11P5_TRUE_N_BINS}"
          continue
        fi
        ...
        cat > "${OVR_FILE}" <<EOF2
{
  ...
  // ---------- l11.5 scan (index 1), same for all centralities ----------
  angRecoBinsVec[0][${L11P5_INDEX}] = std::vector<double>{ ${L11P5_RECO_EDGES} };
  angRecoBinsVec[1][${L11P5_INDEX}] = std::vector<double>{ ${L11P5_RECO_EDGES} };
  angRecoBinsVec[2][${L11P5_INDEX}] = std::vector<double>{ ${L11P5_RECO_EDGES} };

  angMcBinsVecCustom[0][${L11P5_INDEX}] = std::vector<double>{ ${L11P5_TRUE_EDGES} };
  angMcBinsVecCustom[1][${L11P5_INDEX}] = std::vector<double>{ ${L11P5_TRUE_EDGES} };
  angMcBinsVecCustom[2][${L11P5_INDEX}] = std::vector<double>{ ${L11P5_TRUE_EDGES} };
...
EOF2
        ...
        echo "[${RUN_ID}] l11.5 reco max=${L11P5_RECO_MAX}, dense=${L11P5_DENSE_BINS}, Nreco=${L11P5_RECO_N_BINS} | l11.5 true max=${L11P5_TRUE_MAX}"
      ...
      python3 - "${L11P5_RECO_MAX}" "${L11P5_DENSE_BINS}" "${L11P5_MIN_WIDTH}" "${L11P5_TRUE_N_BINS}" <<'PY'
import sys
