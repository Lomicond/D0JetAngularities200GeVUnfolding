...
# -------------------------
L10P5_INDEX=4
# Podle dřívější dohody dávají smysl jen tyto dva pravé okraje.
...
# -------------------------
# Fixed angular binning for all observables except l10.5 (index 4)
# -------------------------
...
# =========================
if [[ ! -f "${SUMMARY}" ]]; then
  printf "run_id\tl10p5_reco_max\tl10p5_dense_bins\tl10p5_reco_n_bins\tl10p5_true_max\tl10p5_true_n_bins\tl10p5_reco_edges\tl10p5_true_edges\n" > "${SUMMARY}"
fi
...
for L10P5_RECO_MAX in "${L10P5_RECO_MAX_LIST[@]}"; do
  for L10P5_DENSE_BINS in "${L10P5_DENSE_BIN_COUNTS[@]}"; do
    ...
    while IFS= read -r L10P5_RECO_EDGES; do
      [[ -n "${L10P5_RECO_EDGES}" ]] || continue

      L10P5_RECO_N_BINS=$(awk -F',' '{print NF-1}' <<< "${L10P5_RECO_EDGES}")

      for L10P5_TRUE_MAX in "${L10P5_TRUE_MAX_LIST[@]}"; do
        L10P5_TRUE_EDGES=$(python3 - "${L10P5_TRUE_MAX}" <<'PY'
import sys
...
PY
        ...
        if (( L10P5_RECO_N_BINS < L10P5_TRUE_N_BINS )); then
          echo "[skip] l10.5 reco max=${L10P5_RECO_MAX}, dense=${L10P5_DENSE_BINS} -> Nreco=${L10P5_RECO_N_BINS} < Ntrue=${L10P5_TRUE_N_BINS}"
          continue
        fi
        ...
        cat > "${OVR_FILE}" <<EOF2
{
  ...
  // ---------- l10.5 scan (index 4), same for all centralities ----------
  angRecoBinsVec[0][${L10P5_INDEX}] = std::vector<double>{ ${L10P5_RECO_EDGES} };
  angRecoBinsVec[1][${L10P5_INDEX}] = std::vector<double>{ ${L10P5_RECO_EDGES} };
  angRecoBinsVec[2][${L10P5_INDEX}] = std::vector<double>{ ${L10P5_RECO_EDGES} };

  angMcBinsVecCustom[0][${L10P5_INDEX}] = std::vector<double>{ ${L10P5_TRUE_EDGES} };
  angMcBinsVecCustom[1][${L10P5_INDEX}] = std::vector<double>{ ${L10P5_TRUE_EDGES} };
  angMcBinsVecCustom[2][${L10P5_INDEX}] = std::vector<double>{ ${L10P5_TRUE_EDGES} };
...
EOF2
        ...
        echo "[${RUN_ID}] l10.5 reco max=${L10P5_RECO_MAX}, dense=${L10P5_DENSE_BINS}, Nreco=${L10P5_RECO_N_BINS} | l10.5 true max=${L10P5_TRUE_MAX}"
      ...
      python3 - "${L10P5_RECO_MAX}" "${L10P5_DENSE_BINS}" "${L10P5_MIN_WIDTH}" "${L10P5_TRUE_N_BINS}" <<'PY'
import sys
